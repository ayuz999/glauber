...
#ifndef __GlauberAnalysisMaker_h__
#define __GlauberAnalysisMaker_h__

class TBranch ;
class TFile ;
class TTree ;
class CentralityMaker ;
class GlauberHistogramMaker ;
class GlauberCumulantHistogramMaker ;
class GlauberTree ;
//#include <map>
#include "TString.h"
...
class GlauberAnalysisMaker {
  public:
    GlauberAnalysisMaker(const TString type="default",
        const TString system = "AuAu_200GeV", const TString outputFileName = "ana.root",
        const TString tableDir = "./"); /// Default constructor
    virtual ~GlauberAnalysisMaker(); /// Default destructor
...
//    std::map<const TString, const UInt_t> mCentralityIdMap ; /// Centrality id for each node
    ...
    GlauberTree* mGlauberTree         ; /// Input ROOT tree (read mode)
    CentralityMaker* mCentralityMaker ; /// Centrality maker

    // Output histograms
    GlauberHistogramMaker* mImpactParameter   ; /// Impact parameter
    GlauberHistogramMaker* mNpart             ; /// Npart
    GlauberHistogramMaker* mNcoll             ; /// Ncoll
    GlauberHistogramMaker* mMultiplicity      ; /// Multiplicity (uncorrected)
    GlauberHistogramMaker* mAreaRP            ; /// Reaction plane area
    GlauberHistogramMaker* mAreaPP            ; /// Participant plane area
    GlauberCumulantHistogramMaker* mEccRP     ; /// Reaction plane eccentricity (2nd order) Npart weight
    GlauberCumulantHistogramMaker* mEccPP[3]  ; /// Participant plane eccentricity (2nd, 3rd and 4th) Npart weight
    GlauberCumulantHistogramMaker* mEccRPM    ; /// Reaction plane eccentricity (2nd order) multiplicity weight
    GlauberCumulantHistogramMaker* mEccPPM[3] ; /// Participant plane eccentricity (2nd, 3rd and 4th) multiplicity weight
